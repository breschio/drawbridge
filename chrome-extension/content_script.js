...
(function() {
  ...
  async function captureScreenshotNative(captureArea, padding = 100) {
    return new Promise((resolve) => {
      window.drawbridgeChrome.sendMessage({ type: 'CAPTURE_SCREENSHOT' }).then((response) => {
        ...
        console.warn('Screenshot message failed:', err);
        resolve(null);
      });
    });
  }
    ...
    try {
      ...
      for (const filename of templateFiles) {
        try {
          ...
          const templateUrl = window.drawbridgeChrome.getURL(`rules-templates/${filename}`);
          ...
          console.error(`❌ Moat: Error details:`, {
            ...
            stack: error.stack,
            templateUrl: window.drawbridgeChrome.getURL(`rules-templates/${filename}`)
          });
...
**Note**: This is a fallback template. The full command documentation should be loaded from the extension files.`;
  ...
  async function testTemplateLoading() {
    ...
    try {
      const templateUrl = window.drawbridgeChrome.getURL('rules-templates/drawbridge-workflow.md');
  ...
  window.drawbridgeChrome.onMessage.addListener((request, sender, sendResponse) => {
